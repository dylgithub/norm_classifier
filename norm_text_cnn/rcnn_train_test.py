...
from rcnn_model import RcnnModel
from rcnn_config import Config
#网络模型的保存文件夹
model_save_location="checkpoints/rcnn"
#网络模型保存的相对路径
...
def train_tes_model(model,tcnn_config):
    # 配置Tensorboard，重新训练时，请将tensorboard文件夹删除，不然图会覆盖
    tensorboard_dir = 'tensorboard/rcnn'
    ...
    _,sen_index,_,sen_pseg_index,one_hot_label = tcnn_data_helper.process_file(
        tcnn_config.file_location,tcnn_config.w2v_model_location,tcnn_config.words_location,tcnn_config.psegs_location,
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(tcnn_config.num_epochs):
            batch_train = tcnn_data_helper.batch_iter(X_train,X_pseg_train,y_train,tcnn_config.batch_size)
            total_batch = 0
        ...
        #训练完之后通过测试集测试模型
        batch_train = tcnn_data_helper.batch_iter(X_test, X_pseg_test,y_test,tcnn_config.batch_size)
        all_test_pred=[]
        for x_batch, x_pseg_batch, y_batch in batch_train:
...
if __name__ == '__main__':
    rcnn_config=Config()
    model=RcnnModel(rcnn_config)
    train_tes_model(model,rcnn_config)